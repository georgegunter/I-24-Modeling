    ...
    def generate_cfg(self, net_params, traffic_lights, routes):
        ...
        if num_traffic_lights > 0:
            if traffic_lights.baseline:
                ...
                for node in tl_properties.values():
                    ...
                    add.append(e)
        #CUSTOM
        if net_params.template != None and 'e1Det' in net_params.template:
            detectors = etree.parse(net_params.template['e1Det'])
            root = detectors.getroot()
            for child in root.iter('e1Detector'):
                if 'output' in net_params.template:
                    child.set('file', net_params.template['output'] + child.get('file'))
                add.append(child)
        #END_CUSTOM    
        ...
        cfg.append(
            _inputs(
                net=self.netfn,
                add=self.addfn,
                #add = "/home/mesto/flow/MyTests/I24-62.add.xml",
                rou=self.roufn,
                gui=self.guifn))
        t = E('time')
